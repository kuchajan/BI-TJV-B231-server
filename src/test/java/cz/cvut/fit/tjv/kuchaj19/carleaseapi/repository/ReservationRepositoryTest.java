...
public class ReservationRepositoryTest {
    ...
    @Autowired
    private ReservationRepository reservationRepository; // test subject

    @Test
    void findByTimeIntervalIntersect() {
        // set up data
        Make make = new Make();
        ...
        // Test the method
        Collection<Reservation> testResult = reservationRepository.findByTimeIntervalIntersect(10L, 20L);

        // Assertions
        Assertions.assertFalse(testResult.isEmpty());
        Assertions.assertEquals(4, testResult.size());
        Assertions.assertFalse(testResult.contains(reservation1));
        Assertions.assertFalse(testResult.contains(reservation2));
        Assertions.assertTrue(testResult.contains(reservation3));
        Assertions.assertTrue(testResult.contains(reservation4));
        Assertions.assertTrue(testResult.contains(reservation5));
        Assertions.assertTrue(testResult.contains(reservation6));
    }
}
